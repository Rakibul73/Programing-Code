...
// last modified: 2022/11/06 23:01:09
...
void solve() {
    ...
    long long countZero = 0, countOne = 0 ,tempOne = 0;
    for (long long i = 0; i < n; i++)
    {
        tempOne = 0;
        if(ss[i] == '1'){
            tempOne++;
            for (long long j = i+1; j < n , ss[j] == '1'; j++)
            {
                tempOne++;
            }
            countOne = max(tempOne, countOne);
        }
        
        
    }
    long long tempZero = 0;
    for (long long i = 0; i < n; i++)
    {
        tempZero = 0;
        if(ss[i] == '0'){
            tempZero++;
            for (long long j = i+1; j < n , ss[j] == '0'; j++)
            {
                tempZero++;
            }
            countZero = max(tempZero, countZero);
        }
    }
    long long dd = zero*one;
    long long bal = max(dd, (countZero*countZero));
    long long sal = max(bal, (countOne*countOne));
    cout<<sal<<"\n";
    return;
