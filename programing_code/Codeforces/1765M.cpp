...
// last modified: 2022/12/01 13:24:51
...
void solve() {
    ...
    cin>>kutta;
    if(kutta %2 == 0) {
        cout<<kutta/2<<" "<<kutta/2<<"\n";
        return;
    }
    long long milf = kutta/2 , big_breast = milf +1;
    long long loli = INFF, boob1, boob2;
    while (milf >= 1)
    {
        long long dudu = lcm_calculate_for_two_num(milf , big_breast);
        if(dudu < loli) {
            loli = dudu;
            boob1 = milf; 
            boob2 = big_breast;
