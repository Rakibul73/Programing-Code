...
// last modified: 2023/06/20 21:18:09
...
void solve() {
    ...
    for (long long i = 0; i < n; i++)
    {
        if (xxx[i] != 0)
        {
            zzzzz.push_back(xxx[i]);
        }
    }

    
    long long count = 0 , edai = 0;
    bool choltase = false;

    for (int i = 0; i < zzzzz.size(); i++) {
        if (zzzzz[i] < 0) {
            if (!choltase) {
                count++;
                choltase = true;
    ...
    for (long long i = 0; i < zzzzz.size(); i++)
    {
