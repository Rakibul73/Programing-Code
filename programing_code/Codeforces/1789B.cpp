...
void solve() {
    ...
    long long temp = n-1;
    for (long long i = 0; i < n/2; i++ , temp--)
    {
        if(ss[i] != ss[temp] && age_silo == false) {
            count = 1;
            continue;
        }
        else if(ss[i] == ss[temp] && age_silo == false && count == 1) {
            age_silo = true;
            continue;
        }
        else if(ss[i] == ss[temp] && age_silo == false && count == 0) {
            
            continue;
        }
        else if(ss[i] != ss[temp] && age_silo == true) {
            cout<<"No\n";
            return;
