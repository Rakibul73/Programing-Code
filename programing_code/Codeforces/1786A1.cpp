...
// last modified: 2023/02/05 18:24:26
...
void solve() {
    long long bal;
    cin>>bal;
    long long sum_a = 1, sum_b = 0;
    bal--;

    bool gggg = true; // b re pala
    for (long long i = 2; bal > 0; i = i + 2)
    {
        if(bal > (i + i + 1) && gggg == true) {
            sum_b = sum_b + i + i + 1;
            gggg = false;
            bal = bal - (i + i + 1);
            continue;
