...
// last modified: 2023/02/10 22:35:07
...
void solve() {
    long long n , ans = 0;
    cin>>n;
    vector<long long>hayre_kopal(n);
    for (long long i = 0; i < n; i++)
    {
        cin>>hayre_kopal[i];
    }
    sort(hayre_kopal.begin(), hayre_kopal.end());
    
    long long counter = 1;
    for (long long i = 0; i < n; i++) {
        if (hayre_kopal[i] >= counter) {
            ans = ans + hayre_kopal[i] - counter;
            counter++;
        }
    }
    cout<<ans<<"\n";
