...
void solve() {
    ...
    long long max_xx = -INFF, max_yy = -INFF, min_xx = INFF, min_yy = INFF;
    for (long long i = 0; i < n; i++)
    {
        if(xx[i] < min_xx) {
            min_xx = xx[i];
    ...
    // cout<<max_xx <<" "<<max_yy <<" "<<min_xx <<" "<< min_yy <<"\n";
    if((max_xx- min_xx) > (max_yy- min_yy)) {
        cout<< (max_xx- min_xx) * (max_xx- min_xx) <<"\n";
        return;
    }
    else {
        cout<< (max_yy- min_yy) * (max_yy- min_yy) <<"\n";
        return;
    }
