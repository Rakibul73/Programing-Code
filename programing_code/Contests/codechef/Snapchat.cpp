...
// last modified: 2022/12/28 23:00:33
...
void solve() {
    long long n;
    cin>>n;
    vector<long long> boobs(n), bra(n);
    ...
    long long milk = 0;
    vector<long long>asol_milk;
    for (long long i = 0; i < n; i++)
    {
        if(bra[i] != 0 && 0 != boobs[i]) {
            milk++;
        }
        else {
            asol_milk.push_back(milk);
            milk = 0;
        }
    }
    asol_milk.push_back(milk);
    sort(asol_milk.begin(), asol_milk.end());
    cout<<asol_milk[asol_milk.size()-1]<<"\n";
    return;
