...
// last modified: 2022/11/30 21:54:45
...
void solve() {
    long long n, xx, yy, xxxx, yyyy;
    cin>>n>>xx>>yy>>xxxx>>yyyy;

    if(xx < 1 && yy > n && xxxx < 1 && yyyy > n) {
        cout<<"0\n";
        return;
    }
    else if(xx < 1 || yy > n) {
        cout<<min(min((n + 1 - xxxx) , (n+1 - yyyy)) , min(xxxx, yyyy))<<"\n";
        return;
    }
    else if(xxxx < 1 || yyyy > n) {
        cout<<min(min((n + 1 - xx) , (n+1 - yy)) , min(xx, yy))<<"\n";
        return;
    }
    long long boobs = min(min((n + 1 - xx) , (n+1 - yy)) , min(xx, yy));
    boobs = boobs + min(min((n + 1 - xxxx) , (n+1 - yyyy)) , min(xxxx, yyyy));
    long long bra = abs(xx - xxxx) + abs(yy - yyyy);
    cout<<min(boobs , bra)<<"\n";
    return;
