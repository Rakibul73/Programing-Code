...
// last modified: 2022/12/07 21:15:42
...
void solve() {
    long long boobs, vagina, kiss, focus = 0, moja = 0;
    cin>>boobs>>vagina>>kiss;
    vector<long long> sex(boobs);
    for (long long i = 0; i < boobs; i++)
    {
        cin>>sex[i];
        if(sex[i] < (vagina - kiss)) {
            focus++;
        }
        else {
            moja = moja + sex[i];
    ...
    cout<<  moja + (focus * vagina) - (focus*kiss)  <<"\n";
    return;
